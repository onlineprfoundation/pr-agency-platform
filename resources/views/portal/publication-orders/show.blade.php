...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Order: {{ $order->publication->name }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 space-y-6">
                ...
                @if($order->canSubmit())
                <form method="POST" action="{{ route('portal.publication-orders.submit', $order) }}" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    <div>
                        ...
                        <div><strong>Documents:</strong>
                            <ul class="mt-2 space-y-1">
                                @foreach($order->documents as $doc)
                                    <li><a href="{{ route('portal.publication-orders.documents.download', [$order, $doc]) }}" class="text-blue-600 hover:underline">{{ $doc->name }}</a></li>
                                @endforeach
                            </ul>
                        </div>
                    @endif
