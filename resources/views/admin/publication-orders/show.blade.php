...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Publication Order #{{ $order->id }}</h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 space-y-6">
                <div class="grid sm:grid-cols-2 gap-4">
                    <p><strong>Publication:</strong> {{ $order->publication->name }}</p>
                    <p><strong>Email:</strong> {{ $order->email }}</p>
                    <p><strong>Source:</strong> {{ $order->source === 'quote_request' ? 'Quote request' : 'Purchase' }}</p>
                ...
                <form method="POST" action="{{ route('admin.publication-orders.update', $order) }}" class="border-t pt-6 space-y-4">
                    @csrf
                    @method('PUT')
                    <div>
