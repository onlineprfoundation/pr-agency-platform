    ...
    <div class="py-16">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 space-y-6">
                <h1 class="text-xl font-bold text-gray-900">Order: {{ $order->publication->name }}</h1>
                <p class="text-sm text-gray-500">Status: {{ ucfirst(str_replace('_', ' ', $order->status)) }}</p>
                @if($order->live_link)
                    <p>Live link: <a href="{{ $order->live_link }}" target="_blank" rel="noopener" class="text-blue-600 hover:underline">{{ $order->live_link }}</a></p>
                @endif

                @if($order->canSubmit())
                <form method="POST" action="{{ route('publication-orders.submit', $order->access_token) }}" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    <div>
